...
export default function RtlCarousel({ slides = defaultSlides }) {
   return (
       <div className="container w-full h-[50vh] sm:h-64  md:h-96 lg:h-130 mx-auto  select-none " dir="rtl">
          <Swiper
          ...
          >
             {slides.map((slide) => (
                 <SwiperSlide key={slide.id} className="flex flex-col justify-center items-center sm:relative w-full h-full overflow-hidden bg-dark-bg ">
                    ...
                    <div className="flex flex-col sm:absolute sm:h-full justify-center sm:items-start text-center sm:text-right sm:pr-12 mt-5 sm:mt-0 z-10 hero-content">
                       <h3 className="text-xl sm:text-3xl font-bold mb-2 md:mb-3 drop-shadow-m max-w-9/12  sm:max-w-7/12 lg:max-w-10/12 leading-12 mx-auto sm:m-0">{slide.title}
